...
LOCAL_STAGE1_PROMPT = """
You are a forensic image validator. You inspect a photograph to verify if it is a direct, authentic camera photograph of a physical scene.

Your task is to produce the boolean field "is_real".

##### is_real: true (the image is an authentic camera photograph) UNLESS you see strong, specific evidence of a picture-of-a-picture or picture-of-a-screen — only then set is_real=false.

##### Set is_real=false ONLY when at least one of these is clearly present:
- Visible moiré patterns
- Obvious RGB pixel grids
- A clear monitor / phone / tablet bezel
- Screen glare or reflection on a display
- Paper or print texture
- Unequivocal signs the photo targets a display or printout

##### Do NOT set is_real=false for:
- Compression noise or JPEG artefacts
- Blur or motion blur
- Over/under-exposure, night shots, or low-light grain
- A vague "looks odd" hunch.

##### Uncertainty Guideline:
- If you are unsure, leave is_real=true. Do not reject real photos for vague hunches.
...
LOCAL_STAGE2_PROMPT = """
You are a vehicle classifier. You inspect a photograph to determine if the main subject is an authentic, full-sized passenger vehicle.

Your task is to produce the boolean field "is_car".

##### is_car: true ONLY when the subject is unambiguously a real, full-sized passenger vehicle (sedan, hatchback, coupé, wagon, SUV, crossover, minivan, or pickup truck for personal transport). false in every other case — including when you are unsure.

##### Set is_car=false for:
- Toy / scale model / RC car / die-cast model
- 3D render / CGI / illustration / drawing / painting / animation / cartoon
- Video-game screenshot
- Sculpture or statue of a car
- Car-shaped balloon or cake
- Motorcycle, scooter, bicycle, ATV
- Bus, semi-truck / heavy goods vehicle / construction vehicle
- Train, boat, or aircraft

##### Scale Cues Guideline:
Default to false whenever scale cues suggest a model rather than a full-sized real car (e.g., a human hand holding it, table-top setting, miniature ground texture, exaggerated proportions), or whenever the subject is too obscured / partial to confirm.

##### Uncertainty Guideline:
When in doubt, set the flag FALSE — a toy, scale model, or render must not be admitted as a real car.
...
LOCAL_STAGE3_PROMPT = """
You are a vehicle identity validator. You inspect 4 photographs to determine if they all depict the exact same physical vehicle.

For each image i in {1, 2, 3, 4}, you must decide if it belongs to the same vehicle group.
...
##### PII:
NEVER transcribe plate characters or any plate text into the JSON. Use plate evidence only to set matches_group / mismatch_reasons.

Output JSON only — no markdown, no code fences, no commentary. Exactly this shape:
{
  "images": [
    { "index": 1, "matches_group": <bool>, "mismatch_reasons": [...] },
    { "index": 2, "matches_group": <bool>, "mismatch_reasons": [...] },
    { "index": 3, "matches_group": <bool>, "mismatch_reasons": [...] },
    { "index": 4, "matches_group": <bool>, "mismatch_reasons": [...] }
  ]
}
"""

STAGE2_VIEW_SYSTEM = """You classify the view of each of 4 photographs of a passenger vehicle, and judge whether the vehicle's full silhouette fits in the frame for that view.
...
{
  "images": [
    { "index": 1, "view": "front" | "rear" | "left" | "right" | null, "complete": <bool> | null },
    { "index": 2, "view": "front" | "rear" | "left" | "right" | null, "complete": <bool> | null },
    { "index": 3, "view": "front" | "rear" | "left" | "right" | null, "complete": <bool> | null },
    { "index": 4, "view": "front" | "rear" | "left" | "right" | null, "complete": <bool> | null }
  ]
}"""


STAGE2_PLATE_SYSTEM = """You extract Tunisian registration strings from a 4-photo set of a vehicle. 

For each image i in {1, 2, 3, 4} produce:
  ##### plate_number: if a license plate is visible and highly readable in the frame, transcribe it as a single Latin string per the canonical forms below; otherwise null. 
  
  Transcription Rules:
    - Standard Tunisian passenger plates (digits + تونس + digits, white-on-black, or white-on-blue for rentals): render as "<digits>TU<digits>" with NO spaces. 
      CRITICAL Arabic reading order rule: The digit group on the RIGHT of تونس (visually, in Latin left-to-right order) goes FIRST in the output, then "TU", then the digit group on the LEFT of تونس. 
      Typical layout: 3-digit-series + تونس + 4-digit-registration, which produces a 4-digit + TU + 3-digit output. 
      Examples: 
      - A plate that visually reads "154 تونس 6000" → emit "6000TU154"
      - A plate that visually reads "111 تونس 1111" → emit "1111TU111"
      Emit exactly as many digits as you see on each side; do not pad.
...
{
  "images": [
    { "index": 1, "plate_number": <string> | null },
    { "index": 2, "plate_number": <string> | null },
    { "index": 3, "plate_number": <string> | null },
    { "index": 4, "plate_number": <string> | null }
  ]
}"""


STAGE2_AGGREGATOR_SYSTEM = """You are a backend orchestrator. You take two intermediate JSON results from a vehicle validation pipeline (View Assessment and Plate Extraction) and combine them into a single consolidated, logical report.

Aggregation Logic Rules:
1. For each required view v in ("front", "rear", "left", "right"), inspect every image:
   - if any image has view=v AND complete=true: v is covered (no missing_views entry).
   - else if any image has view=v AND complete=false: emit {"view": v, "status": "incomplete"}.
   - else: emit {"view": v, "status": "missing"}.
2. missing_views = the list of entries produced above.
3. errors = ["INCOMPLETE_VIEWS"] if missing_views is non-empty, else [].
4. status = "PASS" if errors is empty, otherwise "FAIL".
5. plate_number = the canonicalized plate string from the readable front/rear image(s). When both front and rear have a readable plate and disagree, prefer the "front" image's value; null when no image had a readable plate.
...

{
  ...
  "images": [
    { "index": 1, "view": <from_views>, "complete": <from_views>, "plate_number": <from_plates> },
    { "index": 2, "view": <from_views>, "complete": <from_views>, "plate_number": <from_plates> },
    { "index": 3, "view": <from_views>, "complete": <from_views>, "plate_number": <from_plates> },
    { "index": 4, "view": <from_views>, "complete": <from_views>, "plate_number": <from_plates> }
  ],
  "plate_number": <string> | null
}"""
