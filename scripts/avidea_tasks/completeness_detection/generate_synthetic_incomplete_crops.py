#!/usr/bin/env python3
"""
Generate synthetic incomplete vehicle images by cropping into YOLO detections.
...
def generate(args: argparse.Namespace) -> None:
    ...
    for sample in tqdm(samples, desc="Generating", unit="source"):
        ...
        crop_types = crop_types_for_view(sample.view)

        for variant in range(args.variants_per_source):
            crop_type = rng.choice(crop_types)
            severity = rng.uniform(args.min_severity, args.max_severity)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--min-severity", type=float, default=0.03)
    parser.add_argument("--max-severity", type=float, default=0.15)
...
if __name__ == "__main__":
    ...
    if args.min_severity <= 0:
        raise ValueError("--min-severity must be greater than zero.")

    if args.max_severity <= args.min_severity:
        raise ValueError(
            "--max-severity must be greater than --min-severity."
        )
