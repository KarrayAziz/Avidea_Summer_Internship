    ...
    def extract_plate(self, image_path: str, save_crops_dir: str = None) -> str:
        try:
            ...
            for bbox, text, confidence in ocr_results:
                ...
                # Convert EasyOCR bounding box coordinates to numpy points for OpenCV
                pts = np.array(bbox, np.int32)
                pts = pts.reshape((-1, 1, 2))
                
                # Draw the green bounding polygon for the text segment
                cv2.polylines(segmented_visualization, [pts], isClosed=True, color=(0, 255, 0), thickness=2)
                
                # Overlay the recognized text above its respective box in red
            ...
            # We look for the exact sequence of Noon (\u0646) followed by Teh (\u062a) as a standalone token
            is_rs_plate = False
